...
def run_render(job: dict, args) -> None:
    ...
    run_cmd(render_cmd, f"ComfyUI 渲染（Big5={job['label']})")
...
def run_pairs(job: dict, args) -> None:
    ...
    run_cmd(merge_cmd, f"生成对比图（Big5={job['label']})")


def build_big5_jobs(profile_set: str):
    jobs = []
    if profile_set in ("single", "both"):
        for trait in TRAITS:
    ...
    if profile_set in ("combo", "both"):
        for levels in product(("High", "Low"), repeat=len(TRAITS)):
            pairs = list(zip(TRAITS, levels))
            label = "_".join(f"{TRAIT_ABBR[t]}{'h' if lv=='High' else 'l'}" for t, lv in pairs)
            tokens = ", ".join(f"{t}:{lv}" for t, lv in pairs)
            jobs.append({"label": label, "tokens": tokens, "description": label})
    return jobs


def process_big5_profile(profile: dict,
                         args,
                         categories: Sequence[str],
                         suffix_tag: str,
                         seed: int) -> dict:
    exp_tag = f"{args.exp_prefix}_{profile['label']}"
    ...
    if args.skip_prompts:
        ...
        run_cmd(cmd, f"生成 Prompt（Big5={profile['label']})")
    ...
    if args.skip_normalize:
        ...
        if not need_norm and args.resume:
            ...
            run_cmd(norm_cmd, f"归一化白底图（Big5={profile['label']})")

    return {"label": profile["label"], "exp_tag": exp_tag, "subset": subset_path}


def main():
    parser = argparse.ArgumentParser(description="Big Five 批量自动化（Step1→Step2→Normalize→Render→对比图）")
    parser.add_argument("--profile-set", choices=["single", "combo", "both"], default="single",
                        help="single=五大各自高/低 10 组；combo=五大高低组合 32 组；both=全部。")
    parser.add_argument("--limit-profiles", type=int, default=0,
                        help="仅处理前 N 个 persona（按列表顺序），0 表示不限。")
    ...
    parser.add_argument("--enable-triad", dest="disable_triad", action="store_false",
                        help="启用 triad 路由。")
    parser.add_argument("--exp-prefix", default="big5A_notraid",
                        help="输出 Excel/渲染目录的统一前缀，将自动加上 _<persona>。")
    ...
    parser.add_argument("--big5-profiles", default="big_five_profiles.csv",
    ...
    jobs = build_big5_jobs(args.profile_set)
    if args.limit_profiles and args.limit_profiles > 0:
        jobs = jobs[:args.limit_profiles]

    if not jobs:
        print("[WARN] 没有需要处理的 persona，流程结束。")
        return

    processed_jobs = []
    for idx, profile in enumerate(jobs):
        job = process_big5_profile(profile, args, args.categories, suffix_tag, seed=args.seed + idx * 101)
        processed_jobs.append(job)

    if not processed_jobs:
        print("[WARN] 没有生成任何 prompts，流程结束。")
        return
    ...
    for job in processed_jobs:
        ...
        if args.skip_pairs:
            print(f"[SKIP] 对比图阶段已跳过（Big5={job['label']})")
            continue
